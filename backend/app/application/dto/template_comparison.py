...
class PageComparison:
    ...
    def __post_init__(self) -> None:
        ...
        score_is_valid = (
            isfinite(self.similarity_score)
            and 0.0 <= self.similarity_score <= 1.0
        )
...
class TemplateComparison:
    ...
    def __post_init__(self) -> None:
        if not self.pages:
            raise ValueError(
                "Template comparison must contain at least one page."
            )

        threshold_is_valid = (
            isfinite(self.threshold)
        ...
        if actual_page_numbers != expected_page_numbers:
            raise ValueError(
                "Page numbers must be sequential and start at 1."
            )
