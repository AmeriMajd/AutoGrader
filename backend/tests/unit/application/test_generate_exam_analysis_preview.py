...
class FakeExamRepository:
    ...
    def save(self, exam: Exam) -> None:
        raise AssertionError("Preview generation must not save the exam.")


class FakeFileStorage:
    def __init__(self) -> None:
        self.files = {
            "exams/test/correction.pdf": CORRECTION_CONTENT,
        }
    ...
    def save(self, *, key: str, content: bytes) -> None:
        raise AssertionError("Preview generation must not store a file.")

    def delete(self, *, key: str) -> None:
        raise AssertionError("Preview generation must not delete a file.")
...
def create_use_case(
    ...
    use_case = GenerateExamAnalysisPreview(
        exam_repository=repository,
        file_storage=storage,
        preview_renderer=renderer,
    )

    return use_case, repository, storage, renderer


def test_generates_preview_from_correction_and_regions() -> None:
    exam = create_exam()
    use_case, repository, storage, renderer = create_use_case(exam)
    ...
    assert renderer.calls == [(CORRECTION_CONTENT, exam.answer_regions)]


def test_raises_when_exam_does_not_exist() -> None:
    use_case, _, storage, renderer = create_use_case(None)
...
def test_rejects_missing_source_document(
    ...
    use_case, _, storage, renderer = create_use_case(exam)
...
def test_storage_failure_stops_preview_generation() -> None:
    exam = create_exam()
    use_case, _, storage, renderer = create_use_case(exam)
...
def test_renderer_failure_is_propagated() -> None:
    exam = create_exam()
    use_case, _, storage, renderer = create_use_case(exam)
    renderer.error = RuntimeError("Simulated renderer failure")

    with pytest.raises(
        RuntimeError,
        match="Simulated renderer failure",
    ):
        use_case.execute(exam.id)
